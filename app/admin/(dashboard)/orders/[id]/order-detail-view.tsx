...
export default function OrderDetailView({ orderId }: { orderId: string }) {
  ...
  async function handleUpdateStatus(newStatus: OrderStatus) {
    ...
    setOrder((prev) =>
      prev ? ({ ...prev, ...updates, status: newStatus } as OrderFull) : prev
    );
  ...
  async function handleUpdatePaymentStatus(newPaymentStatus: string) {
    if (!order || paymentUpdating) return;
    setPaymentUpdating(true);

    const { error } = await updatePaymentStatus(order.id, newPaymentStatus);
    ...
    setOrder((prev) =>
      prev ? ({ ...prev, ...updates } as OrderFull) : prev
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <span
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-medium ${status.color} ${status.bg}`}
        >
            ...
            return (
              <div key={step} className="flex items-center flex-1">
                ...
                {i < statusFlow.length - 1 && (
                  <div
                    className={`h-px flex-1 mx-1 -mt-4.5 ${
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="border border-border bg-card p-5">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium border border-red-200 text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
            </div>
        ...
        {/* Right Column: Summary, Contact, Address, Payment */}
        <div className="space-y-6">
          {/* Order Summary */}
          ...
          <div className="border border-border bg-card p-5 space-y-3">
            ...
            <div className="border-t border-border pt-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {["pending", "successful", "failed", "refunded"].map(
                  (pStatus) => (
                    <button
                      key={pStatus}
